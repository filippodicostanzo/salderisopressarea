                        ...
                        <table class="table card-table table-striped">
                            ...
                            @foreach ($items as $key => $item)
                                ...
                                <tr class="@if ($key > 0 && $key % 2) odd @else even @endif">
                                    <td>
                                        @php $extension = strrchr( $item->image, '.');
                                        @endphp
                                        @if ($extension == ".pdf")
                                            <img src="{{asset('images/file-pdf.png')}}" width="50">
                                    ...
                                    <td align="right">
                                        <div class="btn-group">
                                            <a class="btn btn-default btn-xs"
                                               href="{{ route('logos.show',$item->id_code) }}">
                                                <i class="fas fa-eye fa-1x fa-lg" aria-hidden="true"></i>
                                            </a>
                                            <a class="btn btn-default btn-xs"
                                               href="{{ route('logos.edit', $item->id_code) }}">
                                                <i class="fas fa-pencil-alt fa-1x fa-lg" aria-hidden="true"></i>
                                            </a>
                                            {!! Form::open(['method' => 'DELETE','route' => ['logos.destroy', $item->id_code],'style'=>'display:inline']) !!}
